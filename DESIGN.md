...
- **미니멀**: 한 화면에는 한 가지 주요 행동과 한 가지 핵심 결과만 먼저 보인다. 상세, 설정, 보조 지표는 사용자가 필요해진 뒤 드러난다.
- **균형**: 하나의 점수로 사용자를 몰아붙이지 않고, 두 신호의 관계를 함께 보여준다. 예: 만들기/정리, 자동화/판단, 계획/실제, 소비/남김.
- **순환**: 완료 화면은 끝이 아니라 다음 시작점을 남긴다. 모든 주요 흐름은 `기록 → 확인 → 조정 → 다음 루프` 중 어디에 있는지 드러나야 한다.
