# Sam Samuel 디자인

> 이 문서는 제품 디자인의 판단 계층입니다.
> `BRAND.md`는 Sam Samuel이 왜 존재하는지를 정의합니다.
> `DESIGN_SYSTEM.md`는 인터페이스를 어떻게 구현하는지를 정의합니다.
> 이 문서는 구현 디테일에 앞서, 제품이 "맞게 느껴지는가"를 정의합니다.

## 역할

Sam Samuel 디자인은 사람들이 시작하게 만들기 위해 존재합니다.

제품은 복잡함으로 사용자에게 깊은 인상을 주려 해서는 안 됩니다. "무언가 만들고 싶다"와 "지금 시작할 수 있다" 사이의 막연한 거리를 줄여야 합니다. 여기서 좋은 디자인은 장식적인 미니멀리즘이 아닙니다. 넓은 가능성의 공간을 이해한 뒤, 사용자가 움직이도록 돕는 부분만 남기는 — 편집된 명료함입니다.

다음을 판단할 때 이 문서를 사용하세요.

- 새로운 앱 콘셉트
- 제품 흐름과 첫 화면
- 랜딩 페이지와 온보딩
- 카피, 빈 상태(empty state), 에러 메시지
- Threads나 Instagram용 앱 제작 콘텐츠
- 기능이 추가된 뒤에도 제품이 여전히 Sam Samuel답게 느껴지는지

## 참조 문서

- `BRAND.md`: 브랜드 약속, 톤, 감정적 약속, 안티 포지셔닝, 브랜드 상수, 캐릭터 정본
- `DESIGN_SYSTEM.md`: 토큰, 컴포넌트, 접근성, 구현 규칙
- `LLM.md`: 메이커 정체성, 작업 철학, 소통 선호

## 핵심 약속

생산자를 만든다.

이는 디자인이 사람을 다음과 같이 돕는다는 의미입니다.

1. 무엇이 중요한지 본다.
2. 다음 행동을 이해한다.
3. 평가받는 느낌 없이 시도한다.
4. 제품이 대신 해주는 게 아니라 도와준다고 느낄 만큼의 주도권을 유지한다.
5. 내일 다시 오고 싶어진다.

사용자에게는 이미 무언가 만들고 싶은 의지가 있습니다. Sam Samuel 디자인은 거기에 실제로 쓸 수 있는 길을 더합니다.

## 디자인 북극성(North Star)

따뜻한 명료함(Warm clarity).

제품은 곁에 앉은, 조금 더 경험 많은 사람처럼 느껴져야 합니다. 제안하고, 정리하고, 소음을 줄여줄 수는 있지만, 가르치려 들어서는 안 됩니다. 차분하고, 유능하며, 인간적으로 느껴져야 합니다.

화면이 효율적이지만 차갑게 느껴진다면, 방향이 어긋나고 있는 것입니다.
화면이 따뜻하지만 명료하지 않다면, 아직 완성되지 않은 것입니다.
화면이 단순해 보이는 이유가 아무것도 없어서라면, 그것은 편집된 게 아니라 비어 있는 것입니다.

## 제품의 느낌

Sam Samuel 제품은 다음처럼 느껴져야 합니다.

- 사용자와 함께 기다려주는 도구
- 만드는 일이 가능하게 느껴지는 작업 공간
- 다음 단계를 알고 있는 조용한 안내자
- 다시 열기 쉬운 장소
- 도구를 감상하게 하는 게 아니라, 사용자가 스스로의 결과물을 만들도록 돕는 제품

다음처럼 느껴져서는 안 됩니다.

- 차가운 엔터프라이즈 대시보드
- 기능 카탈로그
- 트렌드에 휘둘리는 AI 장난감
- 사용자를 채점하는 선생님
- 실질적 도움 없는 텅 빈 미니멀리즘 페이지
- 따뜻한 색만 입힌 평범한 SaaS 페이지

## 디자인 원칙

### 0. 미니멀 · 균형 · 순환을 화면 문법으로 만든다

Sam Samuel 제품의 기본 컨셉은 **미니멀 · 균형 · 순환**이다. 이 세 단어는 슬로건이 아니라 화면을 만들 때 통과해야 하는 구조 규칙이다.

- **미니멀**: 사용자가 지금 판단할 것만 보여준다. 한 화면에는 한 가지 주요 행동과 한 가지 핵심 결과만 먼저 보이고, 상세·설정·기록·히스토리는 뒤로 보낸다. "다 보여주는 대시보드"는 금지한다.
- **균형**: 상태를 한쪽으로 치우치지 않게 보여준다. 하나의 점수나 3개 이상의 병렬 막대로 사용자를 평가하지 않고, 기본적으로 두 신호의 관계를 한 쌍으로 보여준다. 예: 일/휴식, 기록/실행, 자동화/수동 판단, 생산성/회복, 오늘/내일.
- **순환**: 사용자가 다시 돌아오고 다음 행동으로 넘어가게 만든다. 완료 화면은 끝이 아니라 다음 시작점을 남긴다. 모든 주요 흐름은 `기록 → 해석 → 조정 → 실행 → 다시 기록` 중 어디에 있는지 드러나야 한다.

실제 UI에서는 `지금 할 일 하나`, `현재 기울어진 방향 하나`, `다음에 남길 것 하나`가 첫 화면이나 완료 화면에 나타나야 한다. 이 세 가지가 보이지 않으면 컨셉이 제품에 반영되지 않은 것이다.

제품의 기본 루프는 다음 다섯 단계다.

`Capture → Reflect → Rebalance → Next → Return`

한국어 화면 이름은 다음을 기본값으로 쓴다.

`남기기 → 돌아보기 → 균형 → 다음 손잡이 → 이어서 시작`

영문 루프의 `Rebalance`는 구현 화면명으로 `Balance`를 쓸 수 있다. 단, 이 화면의 역할은 상태를 보여주는 데서 끝나지 않고 치우침을 낮추는 다음 조정안을 남기는 것이다.

여기서 **다음 손잡이**가 순환을 제품 경험으로 바꾸는 핵심 개념이다. 모든 세션은 사용자가 다음에 다시 잡을 수 있는 한 줄을 남기고 닫혀야 한다.

핵심 제품 문장:

사용자가 하루를 적게 남기고, 균형을 확인하고, 내일 다시 시작할 수 있게 한다.

### 1. 첫 행동을 명확하게 만든다

첫 화면은 한 가지 질문에 빠르게 답해야 합니다.

지금 여기서 내가 무엇을 할 수 있는가?

답이 요란할 필요는 없지만, 반드시 보여야 합니다. 사용자가 행동하기 전에 한 문단을 읽어야 한다면, 그 화면은 설명을 너무 많이 짊어지고 있는 것입니다.

좋음:

- 하나의 명확한 주요 행동
- 그 행동에 대한 짧은 이유
- 눈에 보이는 예시, 플레이스홀더, 또는 출발점

피하기:

- 동등한 비중의 여러 CTA
- 추상적인 환영 문구
- 아직 아무것도 없다고만 말하는 빈 상태

### 2. 사용자를 주인공으로 남긴다

인터페이스는 사용자의 콘텐츠, 의도, 또는 결정을 중심에 두어야 합니다.

AI, 자동화, 템플릿은 보조 구조입니다. 제품의 주된 무대가 되어서는 안 됩니다. 사용자는 "이게 내 것을 만드는 걸 도와줬다"고 느껴야지, "도구가 나 대신 뭔가를 만들었다"고 느껴서는 안 됩니다.

좋음:

- 사용자의 재료를 눈에 띄게 보여준다
- 수정은 되돌릴 수 있고 이해할 수 있게 한다
- 제안을 사용자의 목표를 통해 설명한다

피하기:

- 지나치게 마법 같은 생성 흐름
- 제안 뒤의 이유를 숨기는 것
- 불투명한 자동화로 사용자의 판단을 대체하는 것

### 3. 100에서 10으로 편집한다

Sam Samuel의 단순함은 아이디어가 부족해서가 아닙니다. 걸러낸 결과입니다.

무언가를 빼기 전에, 그것이 무엇을 해결하려 했는지 이해하세요. 무언가를 더하기 전에, 그것이 혼란을 줄이는지 아니면 단지 기능을 늘릴 뿐인지 물으세요.

남겨진 10%는 제품을 끌고 갈 만큼 충분히 강하게 느껴져야 합니다.

좋음:

- 더 강력한 기본값을 가진, 더 적은 컨트롤
- 첫 행동 이후에 점진적으로 드러나는 디테일
- 명확한 순간 뒤에 숨겨진 기능의 깊이

피하기:

- 안내가 전혀 없는 얄팍한 MVP
- 사용자가 맥락을 갖기도 전에 노출된 설정
- UI 요소가 적은 것을 무조건 더 낫다고 여기는 태도

### 4. 설명보다 구조를 먼저 쓴다

사용자는 긴 텍스트가 설명하기 전에, 레이아웃·위계·라벨만으로 화면을 이해할 수 있어야 합니다.

카피가 도울 수는 있지만, 카피가 혼란스러운 구조를 구해내서는 안 됩니다.

좋음:

- 관련된 것끼리 가까이 묶는다
- 하나의 요소에 명확한 시각적 우선순위를 준다
- 가능하면 지시문 대신 예시를 쓴다

피하기:

- 긴 설명 블록
- 어디서나 같은 비중의 카드들
- 핵심 이해를 떠안고 있는 툴팁

### 5. 따뜻함을 기능으로 만든다

따뜻함은 장식이 아닙니다. 긴장을 줄여야 합니다.

따뜻한 표면, 둥근 형태, 부드러운 모션, 친근한 카피가 중요한 이유는, 첫 시도를 덜 위험하게 느끼게 하기 때문입니다. 사용자는 처리당하는 게 아니라 초대받는다고 느껴야 합니다.

좋음:

- 부드럽지만 읽기 쉬운 대비
- 촉각적인 버튼과 입력 요소
- 출발 손잡이를 건네는 빈 상태
- 복구 경로가 있는 에러 메시지

피하기:

- 순백의 공허함
- 순흑의 그림자
- 차가운 기본 파란색 포커스 링
- 딱딱한 시스템 메시지

### 6. 다시 돌아오도록 디자인한다

감정적 약속은 "계속 쓰고 싶다."입니다.

좋은 Sam Samuel 제품은 첫 사용에서 사용자를 놀라게 할 필요가 없습니다. 다시 찾을 만큼 편안해져야 합니다. 그러려면 안정적인 내비게이션, 예측 가능한 리듬, 그리고 어제의 작업이 여전히 자리를 갖고 있다는 감각이 필요합니다.

좋음:

- 이전 작업과의 명확한 연속성
- 차분한 시각적 리듬
- 반복 행동으로 가는 빠른 경로
- 작은 진전의 순간들

피하기:

- 일회성 와우 효과
- 끊임없이 새로워지는 레이아웃
- 숨겨진 히스토리
- 다시 쓸 때 처음부터 시작하는 듯한 느낌

## 화면 판단 테스트

색이나 컴포넌트 디테일을 논하기 전에 이 테스트들을 사용하세요.

### 1. 5초 테스트

5초 뒤, 사용자가 다음에 답할 수 있는가:

- 이 화면은 무엇을 위한 것인가?
- 여기서 가장 중요한 것은 무엇인가?
- 다음에 무엇을 해야 하는가?

아니라면, 스타일링 전에 위계를 고치세요.

### 2. 따뜻함 테스트

이 화면이 사람이 사람을 위해 만든 것처럼 느껴지는가?

확인:

- 카피 톤
- 빈 상태
- 에러 상태
- 대기/로딩 상태
- 첫 사용 경험

### 3. 편집 테스트

이것이 100 중에서 골라낸 올바른 10인가?

화면이 비어 보인다면, 그것이 다듬어진 것인지 아니면 그냥 덜 만들어진 것인지 물으세요. 빽빽하다면, 모든 부분이 제자리를 얻을 자격이 있는지 물으세요.

### 4. 주도권 테스트

사용자가 여전히 메이커로 남아 있는가?

제품은 안내하고, 자동화하고, 제안할 수 있지만, 사용자의 판단을 위한 자리를 눈에 보이게 남겨야 합니다.

### 5. 재방문 테스트

사용자가 내일 이걸 다시 열고 싶어할까?

편안함, 연속성, 낮은 마찰을 살피세요. 한 번은 인상적이지만 나중에 피곤해지는 제품은 이 테스트를 통과하지 못합니다.

## 화면 규칙

- 화면당 주요 행동은 하나.
- 가장 강한 가치를 첫 화면(above the fold)에 둔다.
- "어떻게"보다 "왜"를 먼저 보여준다.
- 사용자의 콘텐츠가 시각적 중심이 되게 한다.
- 의도가 분명해진 뒤에 고급 컨트롤을 드러낸다.
- 추상적 지시문보다 예시·미리보기·기본값을 선호한다.
- 빈 상태를 막다른 길이 아니라 출발점으로 쓴다.
- 반복 행동은 늘 닿을 수 있게 둔다.
- 모든 카드·버튼·섹션을 같은 비중으로 만들지 않는다.
- 시각적 위계가 무너지면, 다른 마감은 아직 의미가 없다.

## 비주얼 방향

비주얼 시그니처는 따뜻한 도구(Warm Tool)입니다.

미니멀하고, 촉각적이며, 차분하게 느껴져야 합니다. Braun의 도구적 명료함, 교보의 따뜻한 공간, 츠타야의 큐레이션된 흐름에서 빌려올 수는 있지만, 복고풍·서점 테마·장식적인 느낌이 되어서는 안 됩니다.

여기서 미니멀은 비움이 아니라 **정돈 + 깊이**입니다. 가득 찬 곳에서 필요한 것만 남기고(심플함≠휑함), 남긴 것들을 가지런히 정렬하되, 명암과 층으로 납작하지 않게 만듭니다. 서랍정리·미니멀 패킹처럼 모든 것이 제자리에 있는 상태에서 오는 만족감이 이 브랜드의 미니멀입니다. (2026-07-04 knowledge-lab 취향 분석으로 확정)

### 자연의 언어

디자인은 자연에서 가져옵니다. 분명하게 느낄 수 있는 수준으로. 원천이 되는 세 가지 이미지: **아침 햇살, 나무와 숲, 물과 흐름.** 가장 중요한 요소는 **곡선과 형태**, 그 다음이 색감, 여백 순서입니다.

#### 1. 곡선/형태 (최우선)

| O (이렇게)                                    | X (이렇게 안 함)                              |
| --------------------------------------------- | --------------------------------------------- |
| 모서리에 부드러운 라운딩                        | 직각 모서리                                    |
| 카드마다 미세하게 다른 곡률                      | 모든 요소에 동일한 border-radius               |
| 유기적인 둥근 형태 (조약돌, 잎사귀)              | 완벽한 정원, 정사각형                           |
| 구분선 대신 여백과 곡면으로 영역 분리             | 직선 구분선(hr)으로 영역 자르기                 |
| 아이콘도 둥글고 부드러운 라인                    | 날카로운 각진 아이콘                           |

#### 2. 색감

| O (이렇게)                                    | X (이렇게 안 함)                              |
| --------------------------------------------- | --------------------------------------------- |
| 배경에 미세한 warm 그라데이션                   | 순수 하얀 `#FFFFFF` 배경                       |
| 자연에서 뽑은 톤 (따뜻한 베이지, 크림, 올리브)   | 차가운 순색 (순수 파랑, 순수 회색)              |
| 카드 배경에도 미묘한 톤 차이                     | 모든 카드 동일한 배경색                         |
| 다크모드도 따뜻한 차콜 베이스                    | 순수 검정 `#000000` 다크모드                    |

#### 3. 여백/공간

| O (이렇게)                                    | X (이렇게 안 함)                              |
| --------------------------------------------- | --------------------------------------------- |
| 콘텐츠 사이에 숨 쉴 수 있는 넉넉한 간격          | 빈틈없이 요소로 꽉 찬 화면                     |
| 밀집 영역과 트인 영역이 교차하는 리듬             | 모든 간격이 균일한 기계적 배치                  |
| 화면 가장자리에 충분한 패딩                      | 콘텐츠가 화면 끝까지 붙어 있음                  |
| 중요한 요소 주변에 더 넓은 여백                   | 위계 없이 모든 요소에 같은 여백                 |

### 콘텐츠 비주얼: 가지런히 (Warm Knolling)

썸네일·카드뉴스 등 콘텐츠 이미지는 제품 UI와 축이 다릅니다. UI는 곡선과 유기적 형태가 우선이지만, 콘텐츠 정물은 **격자 정렬**이 우선입니다 — 글 주제를 구성 사물 5~9개로 분해해 90도로 가지런히 놓고, 하나만 크기와 악센트 컬러로 세웁니다(hero). 서랍정리에서 오는 그 만족감이 시각 언어입니다.

정렬은 격자가 하되 따뜻함은 유지합니다: warm cream 배경(#F0EEE9 계열), 아침 사광의 부드러운 그림자, 우드·브라운·크림 저채도, 실사 질감(종이 결·나뭇결·리넨). 납작한 벡터 미니멀은 이 브랜드의 미니멀이 아닙니다.

> 상세 규칙·검증된 프롬프트: knowledge-lab `designs/KNOLLING_STYLE.md` (2026-07-04 확정)

### 공간 코어: 3D 캐릭터 스테이지

Sam Samuel 웹 표면은 점점 작은 살아 있는 작업 공간처럼 느껴져야 합니다. 배경에는 인터랙티브 3D 캐릭터나 scene이 있고, 버튼, 카드, 네비게이션, 상태 컨트롤은 그 위에 떠 있습니다.

이것은 장식용 배경이 아닙니다. 3D 캐릭터 스테이지는 화면의 공간적 주인공입니다. 시스템이 살아 있고, 다가가기 쉽고, 미래지향적으로 느껴지게 하되, 전면 컨트롤은 차분하고 사용 가능해야 합니다.

웹 앱, 대시보드, 도구의 홈, 제품 데모, status형 control surface에서는 이 방향을 먼저 검토합니다.

좋은 방향:

- pointer, scroll, focus, status, page transition 중 하나 이상에 조용히 반응하는 3D 캐릭터나 scene
- 무거운 페이지 섹션보다 floating HUD처럼 배치된 전면 컨트롤
- 모바일에서는 캐릭터가 작고 안정적이며 엄지 조작 영역을 침범하지 않는 구성
- 데스크탑에서는 캐릭터가 더 크고 덜 제한된 위치에서 공간감을 만드는 구성
- full 3D가 과할 때도 video, image sequence, parallax로 stage 감각을 보존하는 fallback

피하기:

- 캐릭터를 평범한 카드 뒤에 붙인 장식 마스코트로 취급하기
- 3D 레이어가 가독성, 성능, 접근성을 해치게 만들기
- 단순한 도구 표면이면 충분한 곳까지 3D spectacle로 만들기
- motion 뒤에 첫 행동을 숨기기
- 목적 있는 character/scene 대신 generic abstract orb나 모호한 분위기 배경 사용하기

전면 규칙은 단순합니다. 사용자는 컨트롤이 살아 있는 시스템 위에 떠 있다고 느껴야지, 시스템이 카드 안에 갇혀 있다고 느끼면 안 됩니다.

### 고정된 방향

- 라이트 모드가 기본 첫인상이다.
- Pretendard가 기본 서체다.
- 형태는 부드럽되, 유치하지 않다.
- 표면에는 깊이가 있되, 무거운 그림자는 없다.
- 배경은 따뜻하고 톤으로 살짝 질감이 있되, 순백은 아니다.
- 모션은 과시가 아니라 사용자와 함께 기다리는 느낌이어야 한다.
- 접근성은 브랜드 약속의 일부다.

### 피하기

- 지배적 인상으로서의 순수 `#FFFFFF`
- 순수 `#000000` 그림자 또는 다크 모드
- 주요 상호작용 색으로서의 차가운 기본 파랑
- 기본 정체성으로서의 보라-파랑 AI SaaS 그라데이션
- 과도한 글래스모피즘
- 장식적인 구체(orb)와 모호한 분위기성 배경
- 장난감처럼 느껴지는 과한 라운딩 컴포넌트
- 제품이 정말로 필요로 하지 않는 한, 빽빽한 엔터프라이즈 테이블 우선 레이아웃

## 카피와 톤

> 말투 정본(존댓말 부드러운 체)과 상황별 O/X 표는 `BRAND.md > 톤앤매너`가 정답입니다. 아래는 화면 카피에 적용할 때의 감각입니다.

Sam Samuel 카피는 부드러운 한국어 존댓말을 씁니다.

곁에 있는 유능한 사람처럼 들려야 합니다.

- "이렇게 해보세요."
- "여기부터 시작하면 돼요."
- "이 부분만 먼저 정리해볼까요?"
- "문제가 생겼어요. 다시 시도할 수 있게 정리해둘게요."

피하기:

- "하십시오"
- "수행하세요"
- "오류가 발생했습니다"
- "사용자는 다음 절차를 따라야 합니다"
- 뻔한 UI를 과하게 설명하는 것
- 사용자를 뒤처진 사람처럼 느끼게 하는 가르치는 톤

### 카피 규칙

- 짧은 문장을 쓴다.
- 행동에 앞서 이유를 먼저 꺼낸다.
- 한 번에 하나의 다음 손잡이만 건넨다.
- 에러는 원인과 복구로 설명한다.
- 추상적인 브랜드 언어보다 구체적인 단어를 선호한다.
- 마감을 더하기 전에 군더더기를 먼저 덜어낸다.

## 앱 제작 콘텐츠 규칙

여행 시대의 앱 제작에서, 디자인은 공유 가능한 증거도 만들어야 합니다.

콘텐츠 루프는 이렇습니다.

1. 여행에서의 관찰 또는 마찰
2. 제품 인사이트
3. 앱 변경
4. 화면 before/after 또는 짧은 데모
5. 확인해볼 다음 사용자 반응

이것은 개발자 일기처럼 보여서는 안 됩니다. 실제 겪은 마찰에서 자라나는 제품을 보여줘야 합니다.

좋은 Threads/Instagram 소재:

- 실제 상황을 담은 사진
- 바뀐 앱의 짧은 화면 녹화
- 담백한 한 문장: "이 장면 때문에 기능 하나를 고쳤습니다."
- 작은 before/after 비교

피하기:

- 긴 기술적 변경 로그
- 평범한 "공개적으로 만들기(building in public)" 게시물
- 사용자 관점의 판단이 진짜 이야기인데 코드를 보여주는 것
- 제품 결정과 동떨어진 여행 사진

## 제품 리뷰 체크리스트

Sam Samuel 제품 화면을 출시하거나 공유하기 전에 확인하세요.

- 첫 행동이 5초 안에 보이는가?
- 화면에 명확한 주인공이 하나 있는가?
- 그냥 빼낸 게 아니라, 올바른 10을 남겼는가?
- 사용자의 콘텐츠나 의도가 중심에 남아 있는가?
- 사용자가 비난받는 느낌 없이 에러에서 복구할 수 있는가?
- 카피가 매뉴얼이 아니라 곁에 있는 선배처럼 들리는가?
- 따뜻함이 혼란을 장식하는 대신 명료함을 돕고 있는가?
- 인터페이스가 구조를 통해 스스로를 설명하는가?
- 핵심 웹 표면이라면, 3D/stage 레이어가 장식이 아니라 공간의 주인공처럼 느껴지는가?
- 사용자가 내일 다시 오고 싶어할까?
- 이것이 여전히 "생산자를 만든다"를 뒷받침하는가?

## 다른 문서와의 관계

문서는 다음 순서로 사용하세요.

1. `BRAND.md`: 이것이 왜 존재해야 하는가?
2. `DESIGN.md`: 이것이 어떤 느낌이어야 하는가?
3. `DESIGN_SYSTEM.md`: 이것을 어떻게 만들어야 하는가?

`DESIGN_SYSTEM.md`가 토큰 규칙을 주는데도 제품이 여전히 차갑게 느껴진다면, `DESIGN.md`로 돌아가세요.
`DESIGN.md`가 모호하게 느껴진다면, `BRAND.md`로 돌아가세요.
둘 다 명확하다면, `DESIGN_SYSTEM.md`를 통해 구현하세요.

## 최종 기준

Sam Samuel 제품은 사용자가 이렇게 생각하게 만들어야 합니다.

지금 시작할 수 있겠다. 그리고 내일 또 오고 싶다.
