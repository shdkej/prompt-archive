...
1. 새 하루 업무 시작 인식
2. **전일 리뷰 존재 여부 확인**
   - **Obsidian**: `~/Obsidian/diary/YYYY-MM.md`에서 어제 날짜(`## MM-DD`) 항목의 `#### 회고` 섹션 확인
   - **전일 리뷰가 없으면 → 먼저 생성**:
     a. "어제({날짜}) 리뷰가 아직 없네요. 먼저 정리하고 시작할게요." 안내
     b. [§ 퇴근 처리](#퇴근-처리) 절차를 **어제 날짜**로 실행 (지연 정리 모드: 제목에 "(늦은 정리)" 표기)
     c. 단, 아침이므로 사용자 확인(9번)은 간소화: 맥락 질문 1~2개만 제시하거나 "0 - 패스"로 바로 저장 가능하도록 안내
     d. 저장 완료 후 아래 3번 단계로 이어감
   - 전일 리뷰가 이미 있으면 → 그대로 읽어서 응답에 포함
...
> 어제 회고 문서가 없으면 먼저 생성한 뒤(동작 2번) 그 결과를 "어제 회고" 섹션에 반영한다.
...
3. 스크립트 출력(프로젝트명, 분류, 사용자 메시지)을 기반으로 각 세션의 핵심 작업 요약
   - **반드시 일일 업무 템플릿의 "오늘의 작업 목록 (Session Log)" 섹션에 테이블 형태로 기록** (업무/개인/기타 분류 유지)
   - 이 섹션은 Obsidian 다이어리와 Google Drive 업로드 양쪽 모두에 포함되어야 함 (사용자가 "내가 뭐했는지" 추적할 수 있도록)
...
> `daily-sessions.py` 출력 기반. 프로젝트별/분류별 세션과 핵심 메시지 1~2줄을 정리한다. 무엇을 했는지 나중에 추적 가능하도록 **반드시 포함**.
