...
- **영향 범위 확인**: 수정 지점이 지목된 작업이라도 변경 전에 ①호출처·참조처(grep) ②같은 패턴이 반복되는 다른 위치 ③짝이 되는 반대편 끝(클라↔서버, 문서↔코드, 로컬↔원격)을 확인합니다. 변경은 지목된 지점으로 최소화하고, 그 밖에서 발견한 이슈는 함께 고치지 않고 `확인한 영향 범위`·`발견된 인접 이슈`로 보고합니다. (`TASTE.md` §1 #인프라쌍)
