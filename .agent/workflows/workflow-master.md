...
**⚠️ 4개 역할 필수 참여 원칙**: 복잡도와 무관하게 Planner, Developer, Marketer, Operator 모두 의견을 제시해야 합니다. 규모가 작을수록 기여 범위는 줄어들지만, 역할 자체를 생략하지 않습니다.
- **Marketer는 항상 참여**: 모든 기능은 결국 사용자에게 보여집니다. 문구, 메시지, UX 카피 등 사용자 접점의 표현을 다듬는 역할은 기능 규모와 무관하게 필요합니다.
- **Operator는 항상 참여**: 만들어진 모든 기능에는 운영이 따릅니다. 운영 효율성, 모니터링, 장애 대응, 운영자 편의성 관점은 설계 단계부터 반영되어야 합니다.

- 간단한 요청의 경우
  - 각 에이전트는 단일한 목표만 수행
  - 기획은 방향성 제시
  - 개발은 가장 간단한 구현
  - 마케팅은 사용자에게 보여질 문구/메시지 다듬기
  - 운영은 운영 효율성 검토 및 가이드라인 확인
- 중간 복잡도의 요청의 경우
  - 각 에이전트별 하나의 파일로 처리
- 복잡한 요청의 경우
  - 모든 산출물 출력
...
- **역할 생략 금지**: 작업이 단순해 보여도 Marketer와 Operator를 빠뜨리지 마세요.
