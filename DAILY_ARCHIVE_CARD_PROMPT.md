...
5. **회고 본문** — 짧게 남긴다. 서로 역할이 다른 **2개 단락**, 합계 3~5문장·시각상 3~5줄로 쓴다. 첫 단락은 그날 실제 장면과 구체를, 둘째 단락은 그 장면이 남긴 판단 또는 다음 장면으로의 전환을 쓴다. 21px/400/line-height 1.58, 부드러운 회색 `#d6d1c8`; 단락 사이는 14px만 띄운다. 사실을 반복하거나 감탄사·격언으로 분량을 채우지 않는다.
6. **내일의 의도** — 화면 맨 아래, 좌우 76px 안쪽. 상단에 1px 구분선. `내일의 의도`는 굵게, 본문은 한 가지 행동만. 25px/400/line-height 1.55, 올리브 계열 `#b8c69e`; 라벨 강조는 `#dfe8ce`.

### 카피 제약

- 제목: 14~28자 권장, 쉼표·줄바꿈으로 리듬을 만들되, 단어를 강제로 쪼개지 않는다. 한글 낱글자 하나가 다음 줄에 홀로 남는 조판(예: `적어 둔` 다음 줄에 `다`)은 실패다. 제목·회고·하단 의도에는 `word-break: keep-all; overflow-wrap: normal; line-break: strict;`를 적용하고, 수동 줄바꿈 뒤에도 한글 한 글자만 남지 않는지 최종 PNG에서 확인한다.
- 회고: **140~180자**. 공백을 포함한 2개 단락의 합계이며, 첫 단락은 관찰·사실, 둘째 단락은 그 사실이 바꾼 판단 또는 다음 전환을 담는다. 140자 미만이면 카드 완성으로 처리하지 않는다. 5줄을 넘기면 중복 문장을 덜고 줄바꿈을 다듬는다. 감탄사·자기계발 격언·AI 운영 용어를 피한다.
...
8. 제목·회고·하단 의도에서 한글 낱글자 하나가 줄 끝 또는 다음 줄에 홀로 떨어지지 않는다. 발견되면 글자를 줄이지 말고 문장을 다듬거나 의미 단위의 줄바꿈을 다시 잡는다.
