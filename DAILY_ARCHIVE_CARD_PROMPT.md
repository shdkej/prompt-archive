...
3. **제목** — 그림 아래 약 58px. 최대 2줄, 58px/500/line-height 1.25, 밝은 크림 `#f8f2e9`. 설명문·슬로건이 아니라 그날의 장면과 판단이 만나는 문장으로 쓴다.
4. **작은 구분선** — 제목 아래 약 31px. 56×2px, 더스티 코랄 `#d88758`.
5. **짧은 회고** — 1~2문장, 시각상 최대 2줄. 29px/400/line-height 1.75, 부드러운 회색 `#d6d1c8`. 사실을 재진술하지 말고 하루가 남긴 감각 또는 다음 장면으로 이어지는 전환을 쓴다.
...
- 회고: 42~72자 권장. 감탄사·자기계발 격언·AI 운영 용어를 피한다.
...
- 텍스트가 2줄을 넘기거나 그림과 겹치면 글자 크기를 무작정 줄이지 않는다. 먼저 카피를 줄이고, 다음으로 줄바꿈을 다듬고, 마지막에 여백을 조정한다.
...
5. 날짜·도시·제목·회고·의도가 하나의 중앙 레일에 정렬되고, 한 카드에 주인공이 하나다.
