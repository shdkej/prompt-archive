...
| `[WORKFLOW:LEARNING]` | 재사용 가능한 교훈 발생 시. `#{카테고리}` 태깅. `~/.claude/TASTE.md`에 적재 |
...
- enabled: true
- log_level: VERBOSE
- log_location: ~/.claude/agent/logs/workflow_{date}.log
```

이 설정이 있으면 workflow-maker가 각 단계마다 위 형식의 로그를 남깁니다.
