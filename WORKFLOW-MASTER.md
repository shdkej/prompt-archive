...
> **CRITICAL — 복잡도 게이트 먼저**: 이 스킬이 호출되면, 에이전트를 띄우기 전에 **작업 복잡도부터 판단**한다.
> sub-agent는 비싸다(각자 글로벌 컨텍스트를 상속해 토큰·시간 소모). 복잡도에 맞는 최소 인원만 투입한다.
>
> | 복잡도 | 투입 방식 |
> | ------ | --------- |
> | **간단** (문서 수정, 설정 변경, 단순 버그, 질문 답변) | **마스터가 직접 처리**하거나 **1개 에이전트만**. 4개 병렬 금지. |
> | **중간** (기능 추가, 리팩토링, API 연동) | 관련된 **2-3개 에이전트만** 병렬 |
> | **복잡** (신규 프로젝트, 아키텍처 변경, 다단계) | **4개 에이전트 전부** 병렬 + 크로스 리뷰 |
>
> "무조건 4개 병렬"은 **복잡 작업에서만** 적용한다. 간단한 일에 4개를 띄우는 것이 오히려 핵심 원칙(미니멀리즘) 위반이다.
> 단, 복잡 작업이라고 판단했으면 그때는 절대 혼자 처리하지 말고 반드시 병렬 위임한다.
