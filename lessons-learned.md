...
### 품질 측정의 유일한 메타 기준 — "다음 결정을 바꾸는가"

- 출처: 고찰 세션 (2026-05-31), `WORKFLOW-MASTER.md`에 내장
- 교훈: 결과 품질을 재는 절대 기준은 없다. 일관되게 작동하는 기준은 단 하나 — **"이 측정이 내 다음 결정을 바꾸는가."** 다음 행동을 못 바꾸는 지표는 전부 허영(vanity)이다. workflow-master의 `[WORKFLOW:METRICS]` 예시였던 "산출물 수·리뷰 건수"가 정확히 허영 지표였고(잘게 쪼개면 부풀려지고, 나와도 뭘 바꿀지 안 떠오름), 4개 게이트로 자기 검증했을 때 그대로 걸러짐.
- 적용: 지표를 세우거나 산출물을 검증할 때 4 게이트 통과 — ① 행동 가능성(나쁘면 뭘 바꿀지 즉시 떠오르나) ② 반증 가능성(실패를 가려내나) ③ 조작 저항성(편법으로 충족 가능한가, Goodhart) ④ 비용 대비. 품질은 단일 점수가 아니므로 정확성·적합성·단순성·지속성·비용을 분리하고, 늦게 드러나는 단순성·지속성은 **지연 측정**(N일 뒤 살아남았나/롤백됐나)으로 따로 남긴다.

---

*마지막 업데이트: 2026-05-31*
*추출 대상 로그: 8개 (2025-02-03 ~ 2026-02-19) + Heartbeat Agent 실행 결과 1건 (monitor-01) + 고찰 세션 1건 (measurement-meta)*
